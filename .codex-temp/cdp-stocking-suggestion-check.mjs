...
if (!port || !url || !outPath) throw new Error('Usage: node cdp-stocking-suggestion-check.mjs <port> <url> <outPath>');
...
const state = await client.send('Runtime.evaluate', {
  expression: `(() => {
    const drawerRect = document.getElementById('detail-drawer')?.getBoundingClientRect();
    return JSON.stringify({
      ...
      metricValues: Array.from(document.querySelectorAll('.metric-value')).map((el) => el.textContent.trim()),
      drawerOpen: document.getElementById('detail-drawer')?.classList.contains('open'),
      drawerLeft: Math.round(drawerRect?.left || 0),
      drawerWidth: Math.round(drawerRect?.width || 0),
      drawerTitle: document.getElementById('drawer-title')?.textContent.trim(),
      drawerRisk: document.getElementById('drawer-risk-text')?.textContent.trim(),
      ...
      hasQueueText: document.body.textContent.includes('优先处理队列'),
      hasAiText: document.body.textContent.includes('AI')
    });
  })()`,
  returnByValue: true
});
